...
function PdfToPowerpointTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF to PowerPoint.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF to PowerPoint. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `pdf-to-powerpoint-output.txt`;
    link.click();
...
export const PdfToPowerpointRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription:
    "PDF to PowerPoint runs entirely in your browser for fast, private processing.",
