...
function ImageUpscalerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using AI Image Upscaler.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by AI Image Upscaler. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-upscaler"}-output.txt`;
    link.click();
...
export const ImageUpscalerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-upscaler",
  slug: "image-upscaler",
  categoryId: "images",
  icon: Scaling,
  component: ImageUpscalerTool,
  layoutDescription: "Increase image resolution without losing detail.",
  layoutDescriptionKey: "tool.image-upscaler.pageDescription",
};
