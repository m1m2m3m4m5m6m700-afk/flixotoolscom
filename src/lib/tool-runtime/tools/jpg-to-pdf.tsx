...
function JpgToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using JPG to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by JPG to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"jpg-to-pdf"}-output.txt`;
    link.click();
...
export const JpgToPdfRuntime: ReadyToolRuntimeDefinition = {
  toolId: "jpg-to-pdf",
  slug: "jpg-to-pdf",
  categoryId: "pdf",
  icon: FileImage,
  component: JpgToPdfTool,
  layoutDescription: "Convert JPG image files into clean PDF documents.",
  layoutDescriptionKey: "tool.jpg-to-pdf.pageDescription",
};
