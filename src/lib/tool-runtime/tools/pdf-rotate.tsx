...
function PdfRotateTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Rotate PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Rotate PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-rotate"}-output.txt`;
    link.click();
...
export const PdfRotateRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-rotate",
  slug: "pdf-rotate",
  categoryId: "pdf",
  icon: RotateCw,
  component: PdfRotateTool,
  layoutDescription: "Rotate PDF pages clockwise or counterclockwise.",
  layoutDescriptionKey: "tool.pdf-rotate.pageDescription",
};
