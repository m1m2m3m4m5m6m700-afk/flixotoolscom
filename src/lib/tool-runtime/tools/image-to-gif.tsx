...
function ImageToGifTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image to GIF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image to GIF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `image-to-gif-output.txt`;
    link.click();
...
export const ImageToGifRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Image to GIF runs entirely in your browser for fast, private processing.",
