...
function MarkdownToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Markdown to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Markdown to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `markdown-to-pdf-output.txt`;
    link.click();
...
export const MarkdownToPdfRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Markdown to PDF runs entirely in your browser for fast, private processing.",
