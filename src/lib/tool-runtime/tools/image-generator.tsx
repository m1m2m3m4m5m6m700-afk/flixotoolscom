...
function ImageGeneratorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using AI Image Generator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by AI Image Generator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-generator"}-output.txt`;
    link.click();
...
export const ImageGeneratorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-generator",
  slug: "image-generator",
  categoryId: "images",
  icon: Sparkles,
  component: ImageGeneratorTool,
  layoutDescription: "Turn a text prompt into original images.",
  layoutDescriptionKey: "tool.image-generator.pageDescription",
};
