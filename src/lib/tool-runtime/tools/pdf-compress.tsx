...
function PdfCompressTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Compress PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Compress PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-compress"}-output.txt`;
    link.click();
...
export const PdfCompressRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-compress",
  slug: "pdf-compress",
  categoryId: "pdf",
  icon: Minimize,
  component: PdfCompressTool,
  layoutDescription: "Reduce PDF file size without sacrificing document quality.",
  layoutDescriptionKey: "tool.pdf-compress.pageDescription",
};
