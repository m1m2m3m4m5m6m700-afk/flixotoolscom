...
function OcrTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using OCR Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by OCR Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"ocr-translator"}-output.txt`;
    link.click();
...
export const OcrTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "ocr-translator",
  slug: "ocr-translator",
  categoryId: "translation",
  icon: ScanText,
  component: OcrTranslatorTool,
  layoutDescription: "Scan text from photos and translate it instantly.",
  layoutDescriptionKey: "tool.ocr-translator.pageDescription",
};
