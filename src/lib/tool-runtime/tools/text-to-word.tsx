...
function TextToWordTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Text to Word.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Text to Word. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `text-to-word-output.txt`;
    link.click();
...
export const TextToWordRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Text to Word runs entirely in your browser for fast, private processing.",
