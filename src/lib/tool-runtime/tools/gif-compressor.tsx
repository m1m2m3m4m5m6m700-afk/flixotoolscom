...
function GifCompressorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using GIF Compressor.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by GIF Compressor. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `gif-compressor-output.txt`;
    link.click();
...
export const GifCompressorRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "GIF Compressor runs entirely in your browser for fast, private processing.",
