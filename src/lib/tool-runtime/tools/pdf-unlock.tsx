...
function PdfUnlockTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF Unlock.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF Unlock. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-unlock"}-output.txt`;
    link.click();
...
export const PdfUnlockRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-unlock",
  slug: "pdf-unlock",
  categoryId: "pdf",
  icon: Unlock,
  component: PdfUnlockTool,
  layoutDescription: "Remove passwords and restrictions from PDF files.",
  layoutDescriptionKey: "tool.pdf-unlock.pageDescription",
};
