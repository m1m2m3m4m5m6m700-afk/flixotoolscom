...
function WatermarkRemoverTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Watermark Remover.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Watermark Remover. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"watermark-remover"}-output.txt`;
    link.click();
...
export const WatermarkRemoverRuntime: ReadyToolRuntimeDefinition = {
  toolId: "watermark-remover",
  slug: "watermark-remover",
  categoryId: "images",
  icon: Eraser,
  component: WatermarkRemoverTool,
  layoutDescription: "Remove watermarks and unwanted overlays from images.",
  layoutDescriptionKey: "tool.watermark-remover.pageDescription",
};
