...
function ImageEditorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using AI Image Editor.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by AI Image Editor. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-editor"}-output.txt`;
    link.click();
...
export const ImageEditorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-editor",
  slug: "image-editor",
  categoryId: "images",
  icon: ImagePlus,
  component: ImageEditorTool,
  layoutDescription: "Edit image brightness, contrast, filters, and styles online.",
  layoutDescriptionKey: "tool.image-editor.pageDescription",
};
