...
function LanguageDetectionTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Language Detector.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Language Detector. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"language-detection"}-output.txt`;
    link.click();
...
export const LanguageDetectionRuntime: ReadyToolRuntimeDefinition = {
  toolId: "language-detection",
  slug: "language-detection",
  categoryId: "translation",
  icon: SearchCode,
  component: LanguageDetectionTool,
  layoutDescription: "Detect language automatically before translating text or speech.",
  layoutDescriptionKey: "tool.language-detection.pageDescription",
};
