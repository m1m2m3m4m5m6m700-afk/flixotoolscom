...
function PdfExtractPagesTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Extract Pages.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Extract Pages. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-extract-pages"}-output.txt`;
    link.click();
...
export const PdfExtractPagesRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-extract-pages",
  slug: "pdf-extract-pages",
  categoryId: "pdf",
  icon: FileStack,
  component: PdfExtractPagesTool,
  layoutDescription: "Save selected pages from a PDF as a new document.",
  layoutDescriptionKey: "tool.pdf-extract-pages.pageDescription",
};
