...
function ImageTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-translator"}-output.txt`;
    link.click();
...
export const ImageTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-translator",
  slug: "image-translator",
  categoryId: "translation",
  icon: Image,
  component: ImageTranslatorTool,
  layoutDescription: "Translate text inside images using OCR and AI translation.",
  layoutDescriptionKey: "tool.image-translator.pageDescription",
};
