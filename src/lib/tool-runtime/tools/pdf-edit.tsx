...
function PdfEditTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Edit PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Edit PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-edit"}-output.txt`;
    link.click();
...
export const PdfEditRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-edit",
  slug: "pdf-edit",
  categoryId: "pdf",
  icon: Edit3,
  component: PdfEditTool,
  layoutDescription: "Add text, annotations, and shapes to PDF files.",
  layoutDescriptionKey: "tool.pdf-edit.pageDescription",
};
