...
function ScreenshotEditorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Screenshot Editor.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Screenshot Editor. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"screenshot-editor"}-output.txt`;
    link.click();
...
export const ScreenshotEditorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "screenshot-editor",
  slug: "screenshot-editor",
  categoryId: "images",
  icon: Crop,
  component: ScreenshotEditorTool,
  layoutDescription: "Annotate, crop, and beautify screenshots with gradients.",
  layoutDescriptionKey: "tool.screenshot-editor.pageDescription",
};
