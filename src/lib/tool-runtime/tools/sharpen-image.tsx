...
function SharpenImageTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Sharpen Image.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Sharpen Image. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"sharpen-image"}-output.txt`;
    link.click();
...
export const SharpenImageRuntime: ReadyToolRuntimeDefinition = {
  toolId: "sharpen-image",
  slug: "sharpen-image",
  categoryId: "images",
  icon: Focus,
  component: SharpenImageTool,
  layoutDescription: "Enhance photo sharpness and clear up blurry edges.",
  layoutDescriptionKey: "tool.sharpen-image.pageDescription",
};
