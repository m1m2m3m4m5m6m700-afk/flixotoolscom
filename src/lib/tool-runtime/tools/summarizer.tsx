...
function SummarizerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Summarizer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Summarizer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"summarizer"}-output.txt`;
    link.click();
...
export const SummarizerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "summarizer",
  slug: "summarizer",
  categoryId: "writing",
  icon: FileText,
  component: SummarizerTool,
  layoutDescription: "Condense long articles and documents into key bullet points.",
  layoutDescriptionKey: "tool.summarizer.pageDescription",
};
