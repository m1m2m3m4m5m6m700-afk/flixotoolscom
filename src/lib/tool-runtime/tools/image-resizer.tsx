...
function ImageResizerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image Resizer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image Resizer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-resizer"}-output.txt`;
    link.click();
...
export const ImageResizerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-resizer",
  slug: "image-resizer",
  categoryId: "images",
  icon: Maximize2,
  component: ImageResizerTool,
  layoutDescription: "Resize images for social media, web, or print.",
  layoutDescriptionKey: "tool.image-resizer.pageDescription",
};
