...
function ArticleGeneratorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Article Generator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Article Generator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"article-generator"}-output.txt`;
    link.click();
...
export const ArticleGeneratorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "article-generator",
  slug: "article-generator",
  categoryId: "writing",
  icon: Newspaper,
  component: ArticleGeneratorTool,
  layoutDescription: "Generate long-form articles with structured headings.",
  layoutDescriptionKey: "tool.article-generator.pageDescription",
};
