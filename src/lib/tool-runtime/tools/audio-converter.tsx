...
function AudioConverterTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Audio Converter.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Audio Converter. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `audio-converter-output.txt`;
    link.click();
...
export const AudioConverterRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Audio Converter runs entirely in your browser for fast, private processing.",
