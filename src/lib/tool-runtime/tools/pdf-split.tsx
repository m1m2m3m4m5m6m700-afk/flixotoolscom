...
function PdfSplitTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Split PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Split PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-split"}-output.txt`;
    link.click();
...
export const PdfSplitRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-split",
  slug: "pdf-split",
  categoryId: "pdf",
  icon: FileDiff,
  component: PdfSplitTool,
  layoutDescription: "Extract individual pages or split PDF documents easily.",
  layoutDescriptionKey: "tool.pdf-split.pageDescription",
};
