...
function RewriteTextTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Rewrite Text.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Rewrite Text. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"rewrite-text"}-output.txt`;
    link.click();
...
export const RewriteTextRuntime: ReadyToolRuntimeDefinition = {
  toolId: "rewrite-text",
  slug: "rewrite-text",
  categoryId: "writing",
  icon: RefreshCw,
  component: RewriteTextTool,
  layoutDescription: "Paraphrase and improve sentences for clarity and style.",
  layoutDescriptionKey: "tool.rewrite-text.pageDescription",
};
