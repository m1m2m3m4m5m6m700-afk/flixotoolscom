...
function CropImageTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Crop Image.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Crop Image. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"crop-image"}-output.txt`;
    link.click();
...
export const CropImageRuntime: ReadyToolRuntimeDefinition = {
  toolId: "crop-image",
  slug: "crop-image",
  categoryId: "images",
  icon: Crop,
  component: CropImageTool,
  layoutDescription: "Crop and frame images precisely in the browser.",
  layoutDescriptionKey: "tool.crop-image.pageDescription",
};
