...
function BlurImageTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Blur Image.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Blur Image. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"blur-image"}-output.txt`;
    link.click();
...
export const BlurImageRuntime: ReadyToolRuntimeDefinition = {
  toolId: "blur-image",
  slug: "blur-image",
  categoryId: "images",
  icon: EyeOff,
  component: BlurImageTool,
  layoutDescription: "Apply soft blur effects to photos and backgrounds.",
  layoutDescriptionKey: "tool.blur-image.pageDescription",
};
