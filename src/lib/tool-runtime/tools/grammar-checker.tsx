...
function GrammarCheckerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Grammar Checker.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Grammar Checker. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"grammar-checker"}-output.txt`;
    link.click();
...
export const GrammarCheckerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "grammar-checker",
  slug: "grammar-checker",
  categoryId: "writing",
  icon: CheckCheck,
  component: GrammarCheckerTool,
  layoutDescription: "Fix spelling, punctuation, and grammatical errors instantly.",
  layoutDescriptionKey: "tool.grammar-checker.pageDescription",
};
