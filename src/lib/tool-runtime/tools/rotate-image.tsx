...
function RotateImageTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Rotate Image.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Rotate Image. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"rotate-image"}-output.txt`;
    link.click();
...
export const RotateImageRuntime: ReadyToolRuntimeDefinition = {
  toolId: "rotate-image",
  slug: "rotate-image",
  categoryId: "images",
  icon: RotateCw,
  component: RotateImageTool,
  layoutDescription: "Rotate images and correct orientation instantly.",
  layoutDescriptionKey: "tool.rotate-image.pageDescription",
};
