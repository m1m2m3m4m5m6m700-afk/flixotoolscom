...
function PdfWatermarkTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Add Watermark.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Add Watermark. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-watermark"}-output.txt`;
    link.click();
...
export const PdfWatermarkRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-watermark",
  slug: "pdf-watermark",
  categoryId: "pdf",
  icon: Stamp,
  component: PdfWatermarkTool,
  layoutDescription: "Add custom text or image watermarks to PDF documents.",
  layoutDescriptionKey: "tool.pdf-watermark.pageDescription",
};
