...
function PdfTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-translator"}-output.txt`;
    link.click();
...
export const PdfTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-translator",
  slug: "pdf-translator",
  categoryId: "translation",
  icon: FileText,
  component: PdfTranslatorTool,
  layoutDescription: "Translate PDF files while keeping formatting intact.",
  layoutDescriptionKey: "tool.pdf-translator.pageDescription",
};
