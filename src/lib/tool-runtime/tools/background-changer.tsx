...
function BackgroundChangerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Background Changer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Background Changer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"background-changer"}-output.txt`;
    link.click();
...
export const BackgroundChangerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "background-changer",
  slug: "background-changer",
  categoryId: "images",
  icon: Wallpaper,
  component: BackgroundChangerTool,
  layoutDescription: "Replace image backgrounds with one click.",
  layoutDescriptionKey: "tool.background-changer.pageDescription",
};
