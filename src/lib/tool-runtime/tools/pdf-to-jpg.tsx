...
function PdfToJpgTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF to JPG.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF to JPG. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-to-jpg"}-output.txt`;
    link.click();
...
export const PdfToJpgRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-to-jpg",
  slug: "pdf-to-jpg",
  categoryId: "pdf",
  icon: ImageDown,
  component: PdfToJpgTool,
  layoutDescription: "Extract PDF pages as high quality JPG images.",
  layoutDescriptionKey: "tool.pdf-to-jpg.pageDescription",
};
