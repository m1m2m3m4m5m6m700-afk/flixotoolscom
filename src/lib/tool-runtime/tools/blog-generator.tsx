...
function BlogGeneratorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Blog Generator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Blog Generator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"blog-generator"}-output.txt`;
    link.click();
...
export const BlogGeneratorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "blog-generator",
  slug: "blog-generator",
  categoryId: "writing",
  icon: BookOpen,
  component: BlogGeneratorTool,
  layoutDescription: "Create SEO-optimized blog posts on any topic.",
  layoutDescriptionKey: "tool.blog-generator.pageDescription",
};
