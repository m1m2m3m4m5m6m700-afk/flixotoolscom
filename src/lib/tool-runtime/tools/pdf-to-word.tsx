...
function PdfToWordTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF to Word.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF to Word. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-to-word"}-output.txt`;
    link.click();
...
export const PdfToWordRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-to-word",
  slug: "pdf-to-word",
  categoryId: "pdf",
  icon: FileUp,
  component: PdfToWordTool,
  layoutDescription: "Convert PDF documents into editable Word files.",
  layoutDescriptionKey: "tool.pdf-to-word.pageDescription",
};
