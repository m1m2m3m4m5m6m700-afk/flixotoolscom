...
function WebsiteTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Website Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Website Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"website-translator"}-output.txt`;
    link.click();
...
export const WebsiteTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "website-translator",
  slug: "website-translator",
  categoryId: "translation",
  icon: Globe,
  component: WebsiteTranslatorTool,
  layoutDescription: "Translate entire websites and web content in one click.",
  layoutDescriptionKey: "tool.website-translator.pageDescription",
};
