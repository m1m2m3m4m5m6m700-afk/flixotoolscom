...
function PowerpointToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PowerPoint to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PowerPoint to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"powerpoint-to-pdf"}-output.txt`;
    link.click();
...
export const PowerpointToPdfRuntime: ReadyToolRuntimeDefinition = {
  toolId: "powerpoint-to-pdf",
  slug: "powerpoint-to-pdf",
  categoryId: "pdf",
  icon: Presentation,
  component: PowerpointToPdfTool,
  layoutDescription: "Convert PPT and PPTX presentations into PDF slides.",
  layoutDescriptionKey: "tool.powerpoint-to-pdf.pageDescription",
};
