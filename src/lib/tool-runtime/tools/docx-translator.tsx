...
function DocxTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using DOCX Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by DOCX Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"docx-translator"}-output.txt`;
    link.click();
...
export const DocxTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "docx-translator",
  slug: "docx-translator",
  categoryId: "translation",
  icon: FileSpreadsheet,
  component: DocxTranslatorTool,
  layoutDescription: "Translate Word documents and preserve layout and styles.",
  layoutDescriptionKey: "tool.docx-translator.pageDescription",
};
