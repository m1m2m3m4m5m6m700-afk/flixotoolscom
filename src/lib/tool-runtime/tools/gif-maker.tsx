...
function GifMakerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using GIF Maker.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by GIF Maker. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `gif-maker-output.txt`;
    link.click();
...
export const GifMakerRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "GIF Maker runs entirely in your browser for fast, private processing.",
