...
function SubtitleTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Subtitle Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Subtitle Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"subtitle-translator"}-output.txt`;
    link.click();
...
export const SubtitleTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "subtitle-translator",
  slug: "subtitle-translator",
  categoryId: "translation",
  icon: Subtitles,
  component: SubtitleTranslatorTool,
  layoutDescription: "Translate SRT and VTT subtitle files line by line.",
  layoutDescriptionKey: "tool.subtitle-translator.pageDescription",
};
