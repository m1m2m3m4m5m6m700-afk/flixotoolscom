...
function ColorPickerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Color Picker.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Color Picker. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"color-picker"}-output.txt`;
    link.click();
...
export const ColorPickerRuntime: ReadyToolRuntimeDefinition = {
  toolId: "color-picker",
  slug: "color-picker",
  categoryId: "images",
  icon: Pipette,
  component: ColorPickerTool,
  layoutDescription: "Pick exact HEX and RGB color codes from any uploaded photo.",
  layoutDescriptionKey: "tool.color-picker.pageDescription",
};
