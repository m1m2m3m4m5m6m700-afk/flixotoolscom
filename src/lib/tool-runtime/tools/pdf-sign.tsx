...
function PdfSignTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF Sign.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF Sign. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-sign"}-output.txt`;
    link.click();
...
export const PdfSignRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-sign",
  slug: "pdf-sign",
  categoryId: "pdf",
  icon: Signature,
  component: PdfSignTool,
  layoutDescription: "Add electronic signatures to PDF contracts and forms.",
  layoutDescriptionKey: "tool.pdf-sign.pageDescription",
};
