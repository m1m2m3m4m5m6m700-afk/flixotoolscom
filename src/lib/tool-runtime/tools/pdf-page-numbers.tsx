...
function PdfPageNumbersTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF Page Numbers.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF Page Numbers. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `pdf-page-numbers-output.txt`;
    link.click();
...
export const PdfPageNumbersRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "PDF Page Numbers runs entirely in your browser for fast, private processing.",
