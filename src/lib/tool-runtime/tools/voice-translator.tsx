...
function VoiceTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Voice Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Voice Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"voice-translator"}-output.txt`;
    link.click();
...
export const VoiceTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "voice-translator",
  slug: "voice-translator",
  categoryId: "translation",
  icon: Mic,
  component: VoiceTranslatorTool,
  layoutDescription: "Translate spoken words in real time from one language to another.",
  layoutDescriptionKey: "tool.voice-translator.pageDescription",
};
