...
function ImageToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-to-pdf"}-output.txt`;
    link.click();
...
export const ImageToPdfRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-to-pdf",
  slug: "image-to-pdf",
  categoryId: "images",
  icon: FileImage,
  component: ImageToPdfTool,
  layoutDescription: "Convert JPG and PNG photos into a single PDF document.",
  layoutDescriptionKey: "tool.image-to-pdf.pageDescription",
};
