...
function ImageConverterTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image Converter.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image Converter. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-converter"}-output.txt`;
    link.click();
...
export const ImageConverterRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-converter",
  slug: "image-converter",
  categoryId: "images",
  icon: FileType,
  component: ImageConverterTool,
  layoutDescription: "Convert images between JPG, PNG, WEBP, and AVIF formats.",
  layoutDescriptionKey: "tool.image-converter.pageDescription",
};
