...
function PdfToTextTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF to Text.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF to Text. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `pdf-to-text-output.txt`;
    link.click();
...
export const PdfToTextRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "PDF to Text runs entirely in your browser for fast, private processing.",
