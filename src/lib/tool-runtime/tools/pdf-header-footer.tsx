...
function PdfHeaderFooterTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF Header Footer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF Header Footer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `pdf-header-footer-output.txt`;
    link.click();
...
export const PdfHeaderFooterRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription:
    "PDF Header Footer runs entirely in your browser for fast, private processing.",
