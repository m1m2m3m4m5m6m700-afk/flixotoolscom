...
function PdfOcrTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using PDF OCR.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by PDF OCR. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-ocr"}-output.txt`;
    link.click();
...
export const PdfOcrRuntime: ReadyToolRuntimeDefinition = {
  toolId: "pdf-ocr",
  slug: "pdf-ocr",
  categoryId: "pdf",
  icon: Scan,
  component: PdfOcrTool,
  layoutDescription: "Convert scanned PDF documents into searchable text.",
  layoutDescriptionKey: "tool.pdf-ocr.pageDescription",
};
