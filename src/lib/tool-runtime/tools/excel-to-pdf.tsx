...
function ExcelToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Excel to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Excel to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"excel-to-pdf"}-output.txt`;
    link.click();
...
export const ExcelToPdfRuntime: ReadyToolRuntimeDefinition = {
  toolId: "excel-to-pdf",
  slug: "excel-to-pdf",
  categoryId: "pdf",
  icon: Table,
  component: ExcelToPdfTool,
  layoutDescription: "Convert XLS and XLSX spreadsheets into printable PDFs.",
  layoutDescriptionKey: "tool.excel-to-pdf.pageDescription",
};
