...
function WordToPdfTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Word to PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for Word to PDF:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Word to PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"word-to-pdf"}-output.txt`;
    link.click();
...
export const WordToPdfRuntime: ReadyToolRuntimeDefinition = {
  toolId: "word-to-pdf",
  slug: "word-to-pdf",
  categoryId: "pdf",
  icon: FileText,
  component: WordToPdfTool,
  layoutDescription: "Convert DOC and DOCX Word files into PDF format.",
  layoutDescriptionKey: "tool.word-to-pdf.pageDescription",
};
