...
function VideoTrimmerTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Video Trimmer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Video Trimmer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `video-trimmer-output.txt`;
    link.click();
...
export const VideoTrimmerRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Video Trimmer runs entirely in your browser for fast, private processing.",
