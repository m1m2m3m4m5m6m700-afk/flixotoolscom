...
function FaceBlurTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Face Blur.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Face Blur. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"face-blur"}-output.txt`;
    link.click();
...
export const FaceBlurRuntime: ReadyToolRuntimeDefinition = {
  toolId: "face-blur",
  slug: "face-blur",
  categoryId: "images",
  icon: UserX,
  component: FaceBlurTool,
  layoutDescription: "Automatically detect and blur faces in photos for privacy.",
  layoutDescriptionKey: "tool.face-blur.pageDescription",
};
