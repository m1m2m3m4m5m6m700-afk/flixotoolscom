...
function LargeTextTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Large Text Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Large Text Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"large-text-translator"}-output.txt`;
    link.click();
...
export const LargeTextTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "large-text-translator",
  slug: "large-text-translator",
  categoryId: "translation",
  icon: FileText,
  component: LargeTextTranslatorTool,
  layoutDescription: "Translate long-form content, web pages, and large blocks of text quickly.",
  layoutDescriptionKey: "tool.large-text-translator.pageDescription",
};
