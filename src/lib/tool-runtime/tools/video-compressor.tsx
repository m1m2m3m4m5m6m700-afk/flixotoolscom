...
function VideoCompressorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Video Compressor.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Video Compressor. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `video-compressor-output.txt`;
    link.click();
...
export const VideoCompressorRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Video Compressor runs entirely in your browser for fast, private processing.",
