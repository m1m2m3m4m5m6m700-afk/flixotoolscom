...
function DocumentTranslatorTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Document Translator.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Document Translator. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"document-translator"}-output.txt`;
    link.click();
...
export const DocumentTranslatorRuntime: ReadyToolRuntimeDefinition = {
  toolId: "document-translator",
  slug: "document-translator",
  categoryId: "translation",
  icon: FileCheck,
  component: DocumentTranslatorTool,
  layoutDescription: "Translate whole documents while keeping their layout.",
  layoutDescriptionKey: "tool.document-translator.pageDescription",
};
