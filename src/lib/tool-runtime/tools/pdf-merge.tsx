...
function PdfMergeTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Merge PDF.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Merge PDF. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"pdf-merge"}-output.txt`;
    link.click();
...
export const PdfMergeRuntime: ReadyToolRuntimeDefinition = {
  ...
  layoutDescription: "Combine multiple PDF files into one structured document.",
  layoutDescriptionKey: "tool.pdf-merge.pageDescription",
};
