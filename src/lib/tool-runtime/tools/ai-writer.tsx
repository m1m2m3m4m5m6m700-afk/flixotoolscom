...
function AiWriterTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using AI Writer.`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by AI Writer. Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"ai-writer"}-output.txt`;
    link.click();
...
export const AiWriterRuntime: ReadyToolRuntimeDefinition = {
  toolId: "ai-writer",
  slug: "ai-writer",
  categoryId: "writing",
  icon: PenTool,
  component: AiWriterTool,
  layoutDescription: "Draft essays, articles, and marketing copy with AI.",
  layoutDescriptionKey: "tool.ai-writer.pageDescription",
};
