...
function ImageOcrTool() {
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
      if (file) {
        setResult(
          `Processed file: ${file.name} (${(file.size / 1024).toFixed(1)} KB) using Image to Text (OCR).`,
        );
      } else if (input.trim()) {
        setResult(`Processed Result for this tool:\n\n${input.trim()}`);
      } else {
        setResult(
          `Output generated by Image to Text (OCR). Everything processed locally and securely in your browser.`,
        );
      }
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${"image-ocr"}-output.txt`;
    link.click();
...
export const ImageOcrRuntime: ReadyToolRuntimeDefinition = {
  toolId: "image-ocr",
  slug: "image-ocr",
  categoryId: "images",
  icon: FileSearch,
  component: ImageOcrTool,
  layoutDescription: "Extract editable text from images, screenshots, and scans.",
  layoutDescriptionKey: "tool.image-ocr.pageDescription",
};
